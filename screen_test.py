...
def int_to_bits_bitwise(byte:int):
    bits_list = []
...
def update_screen_row(screen_arr:list[list[int]], x:int, y:int, sprite_row:int) -> bool:
    if y >= len(screen_arr):
        raise ValueError("Out of range when drawing screen.")
    
    vf = False
    screen_row = screen_arr[y]
    bits_list = int_to_bits_bitwise(sprite_row)
...
def update_screen(screen_arr:list[list[int]], x:int, y:int, sprite:bytearray) -> bool:
    ...
    for row in sprite:
        if y >= len(screen_arr):
            break
        vf = update_screen_row(screen_arr, x, y, row)
        y += 1
